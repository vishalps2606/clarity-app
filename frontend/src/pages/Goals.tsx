...
export default function Goals() {
  ...
  const fetchData = async () => {
    try {
      // Parallel Fetch: Get Goals AND Tasks
    ...
    <AppLayout>
      <header className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-text-secondary">High-level objectives and progress.</p>
        </div>
        <Button 
      ...
      {loading ? (
        <div className="text-neon-blue font-mono animate-pulse">Aligning satellites...</div>
      ) : goals.length === 0 ? (
        <div className="flex flex-col items-center justify-center p-12 border border-dashed border-border rounded-lg text-center">
            ...
            <h3 className="text-xl font-bold text-text-primary">No Strategy Defined</h3>
            <p className="text-text-secondary mb-6 max-w-md">
                You are operating tactically without a strategic layer. Define a Goal to group your tasks.
            </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {goals.map((goal) => {
            ...
            const completed = goalTasks.filter((t: any) => t.status === 'DONE').length;
